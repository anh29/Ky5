	...
	for(int i=0; i<m; i++){
		for(int j=0; j<n; j++){
			if(fabs(A[i][j]) < 0.00001) 
				cout << setw(10) << setprecision(3) << 0;
				else cout << setw(10) << setprecision(3) << A[i][j];
		}
			cout << endl;
	}
}
...
void singularValueDecomposition(int m,int n,m2c A){
	...
	cout << endl << "Singular values decomposition(SVD): A = U*xichMa*V^T";
	cout<< endl<<endl<< "Matrix U = " <<endl;
	outputMatrix(m,m,U);
	cout<< endl<<"Matrix xichMa ="<<endl;
    outputMatrix(m,n,W);
